  ...
  <v-container fluid>
    <div>
      ...
      >
        <template v-slot:top>
          <v-toolbar>
            <v-toolbar-title>Daftar Tipe Kerjasama</v-toolbar-title>
          </v-toolbar>
          <v-divider></v-divider>
...
export default {
  data: () => ({
    ...
    tipeKerjasama: [],
    headers: [
      { text: "Nama Tipe Kerjasama", value: "nama" },
  ...
  computed: {
    formTitle() {
      return this.editedIndex === -1 ? "Tambah Tipe Kerjasama" : "Ubah Tipe Kerjasama";
    },
    roleCodeErrors() {
      const errors = [];
  ...
  methods: {
    ...
    postData() {
      apiServer
        .post("/tipe-kerjasama", this.editedItem)
        .then(response => {
          this.getData();
    ...
    deleteData(id) {
      apiServer
        .delete("/tipe-kerjasama/" + id + "/" + this.$store.state.user.name)
        .then(response => {
          this.getData();
    ...
    updateData() {
      apiServer
        .put("/tipe-kerjasama", this.editedItem)
        .then(response => {
          this.getData();
    ...
    editItem(item) {
      ...
      this.editedIndex = this.tipeKerjasama.indexOf(item);
