        ...
        this.$store.dispatch('login', { username, password })
          ...
          .then(
            fetch("https://api.ipify.org?format=json")
              .then(x => x.json())
              .then(({ ip }) => {
                this.userObject.lastIp = ip
                this.userObject.id = this.$store.state.user.userId
                apiServer
                  .put("/userweb-login-info", this.userObject)
                  .then(response => {
                    console.log("Success update login info..");
                  })
                  .catch(error => {
