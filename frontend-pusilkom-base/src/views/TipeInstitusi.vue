  ...
  <v-container fluid>
    <div>
      ...
      >
        <template v-slot:top>
          <v-toolbar>
            <v-toolbar-title>Daftar Tipe Institusi</v-toolbar-title>
          </v-toolbar>
          <v-divider></v-divider>
...
export default {
  data: () => ({
    ...
    tipeInstitusi: [],
    headers: [
      { text: "Nama Tipe Institusi", value: "nama" },
  ...
  computed: {
    formTitle() {
      return this.editedIndex === -1 ? "Tambah Tipe Institusi" : "Ubah Tipe Institusi";
    },
    roleCodeErrors() {
      const errors = [];
  ...
  methods: {
    ...
    postData() {
      apiServer
        .post("/tipe-institusi", this.editedItem)
        .then(response => {
          this.getData();
    ...
    deleteData(id) {
      apiServer
        .delete("/tipe-institusi/" + id + "/" + this.$store.state.user.name)
        .then(response => {
          this.getData();
    ...
    updateData() {
      apiServer
        .put("/tipe-institusi", this.editedItem)
        .then(response => {
          this.getData();
    ...
    editItem(item) {
      ...
      this.editedIndex = this.tipeInstitusi.indexOf(item);
