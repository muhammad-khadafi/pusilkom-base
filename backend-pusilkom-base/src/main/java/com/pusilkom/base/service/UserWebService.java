...
public class UserWebService {
    ...
    public List<UserwebDTO> getUserweb() {
        ...
        for (Userweb userweb :
            ...
            if (userwebDTO.getIdInstitusi() != null) {
                Organisasi organisasi = organisasiMapper.selectByPrimaryKey(userweb.getIdInstitusi());
                userwebDTO.setInstitusi(organisasi.getNama());
            }
...
//        List<Komentar> komentars = komentarMapper.selectByExample(komentarExample);
        ...
        userwebHasRoleExample.createCriteria().andIdUserwebEqualTo(id.intValue());
...
//            || !komentars.isEmpty()
    ...
    public String getUserwebFoto(Integer id) {
        ...
        if (userweb.getFoto() != null) {
            ...
            try {
                ...
                System.out.println(userweb.getUsername() + " photo doesn't exist..");
            }
    ...
    public void userWebResetPassword(UserwebDTO userwebDTO)
        ...
        String bodyEmail = "<h3>Hi " + userweb.getNama() + ",</h3>" +
            ...
            "<p>Password : " + newPass + "</p>" +
            "<br/><p>Salam,</p>" +
            "<p>Admin SIKERJASAMA</p>";


//        customEmailSenderService.sendEmailHtml(userweb.getEmail(),
//                "SIKERJASAMA - RESET PASSWORD", bodyEmail);
        System.out.println(newPass);
    }
    ...
    public List<Userweb> getUserwebByOrganization(Integer organizationId) {
        UserwebExample userwebExample = new UserwebExample();
        userwebExample.createCriteria().andIdInstitusiEqualTo(organizationId);
        userwebExample.setOrderByClause("username");
    ...
    public void simpleUpdateUserweb(UserwebDTO userwebDTO) {
        userwebMapper.updateByPrimaryKeySelective(toUserweb(userwebDTO));
    }

    public List<UserwebDTO> getUserLoginList() {
        List<UserwebDTO> userwebDTOList = userwebMapper.getUserLoginList();

        userwebDTOList.stream().forEach((userweb) -> {
            userweb.setLastLoginStr(new SimpleDateFormat("dd-MM-yyyy HH:mm:ss").format(userweb.getLastLogin()));
        });

        return userwebDTOList;
    }

    public UserwebDTO getProfilPengguna(Integer idUser) {
        UserwebDTO userwebDTO = userwebMapper.getProfilPengguna(idUser);

        if (userwebDTO.getLastLogin() != null)
            userwebDTO.setLastLoginStr(new SimpleDateFormat("dd-MM-yyyy HH:mm:ss").format(userwebDTO.getLastLogin()));
        if (userwebDTO.getLastLogout() != null)
            userwebDTO.setLastLogoutStr(new SimpleDateFormat("dd-MM-yyyy HH:mm:ss").format(userwebDTO.getLastLogout()));
        if (userwebDTO.getKodeSso() == null || userwebDTO.getKodeSso().equals(""))
            userwebDTO.setKodeSso("-");
        if (userwebDTO.getLastLoginStr() == null || userwebDTO.getLastLoginStr().equals(""))
            userwebDTO.setLastLoginStr("-");
        if (userwebDTO.getLastLogoutStr() == null || userwebDTO.getLastLogoutStr().equals(""))
            userwebDTO.setLastLogoutStr("-");
        if (userwebDTO.getNip() == null || userwebDTO.getNip().equals(""))
            userwebDTO.setNip("-");
        if (userwebDTO.getNama() == null || userwebDTO.getNama().equals(""))
            userwebDTO.setNama("-");
        if (userwebDTO.getEmail() == null || userwebDTO.getEmail().equals(""))
            userwebDTO.setEmail("-");
        if (userwebDTO.getInstitusi() == null || userwebDTO.getInstitusi().equals(""))
            userwebDTO.setInstitusi("-");
