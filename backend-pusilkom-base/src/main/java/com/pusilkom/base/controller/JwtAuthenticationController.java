...
public class JwtAuthenticationController {
    ...
    public ResponseEntity<?> createAuthenticationToken(@RequestBody JwtRequest authenticationRequest) throws Exception {
        ...
        List<Menu> menus = userDetailsService.getMenus(roles.get(0).getId().longValue());

        Institusi institusi = userDetailsService.getInstitusi(userweb.getIdInstitusi());
        ...
        response.setOrganizationId(userweb.getIdInstitusi());
        response.setMenus(menus);
        response.setInstitusi(institusi);
        response.setInstitutionId(institusi.getId());
